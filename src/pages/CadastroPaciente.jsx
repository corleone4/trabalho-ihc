...
// ── Field: label text-xs para WCAG 1.4.4 ─────────────────────────────────────
...
// ── inputCls: tema claro consistente ─────────────────────────────────────────
// CORREÇÃO PRINCIPAL: removido bg-slate-800/60 text-slate-100 (era dark mode)
    ...
    <div className="flex flex-col items-center px-4 py-2">
      <div className="max-w-xl w-full flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          {/* Botão Voltar com aria-label descritivo */}
          <button
            type="button"
          ...
          <div className="flex items-center gap-3">
            <div
              className="flex items-center justify-center w-11 h-11 rounded-2xl
                         ...
                         text-white shadow-md shadow-indigo-500/25 shrink-0"
              aria-hidden="true"
            >
              <Icon.User />
            </div>
            <div>
              {/* CORREÇÃO: text-slate-100 → text-slate-800 (legível em fundo claro) */}
              <h1 className="text-xl font-bold text-slate-800 leading-tight">
                Cadastro de Paciente
              </h1>
              <p className="text-sm text-slate-500 mt-0.5">
                Registre um novo paciente na clínica
              </p>
            </div>
          </div>
        </div>
        ...
        {sucesso ? (
          ...
          >
            ...
            <p className="text-base font-bold text-slate-800">Paciente cadastrado!</p>
            <p className="text-sm text-slate-500">Redirecionando…</p>
          </div>

        ) : (
          /* ── Formulário ───────────────────────────────────────────────── */
          // CORREÇÃO: removido border-slate-800 bg-slate-900/80 (dark) →
          //           border-slate-200 bg-white shadow-lg (light)
          <form
            onSubmit={cadastrar}
            noValidate
          ...
          >
            ...
            {/*
              CORREÇÕES:
              - border-b border-slate-800 → border-slate-100
              - Linha inativa bg-slate-800 → bg-slate-200 (visível em branco)
              - Etapa futura: bg-slate-800 border-slate-700 → bg-slate-100 border-slate-200
              - aria-label no tablist
            */}
            <div
              ...
              {STEPS.map((label, i) => {
                ...
                return (
                  <div key={label} className="flex-1 flex flex-col items-center gap-1.5 relative">
                    ...
                    >
                      {isDone
                        ? <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" aria-hidden="true"><path d="M20 6 9 17l-5-5" /></svg>
                        : i + 1}
                    </button>
                    <span className={[
              ...
              {step === 0 && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Field id="nascimento" label="Data de nascimento" icon={Icon.Calendar}>
                      <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">
                        <Icon.Calendar />
                      </span>
                      <input
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Field id="sexo" label="Sexo biológico">
                      <select
                        id="sexo" name="sexo"
            ...
            {/*
              CORREÇÕES:
              - border-t border-slate-800 → border-slate-100
              - Cancelar: hover:text-slate-300 (invisível em branco) → hover:text-slate-700
              - Voltar: border-slate-700 text-slate-400 hover:bg-slate-800 (dark) →
                        border-slate-300 text-slate-600 bg-white hover:bg-slate-50 (light)
            */}
